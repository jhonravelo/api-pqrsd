...
router.get("/all", async (req, res) => {
  try {
    ...
    const [[Types]] = await userModel.getAll();
    if (!Types)
    ...
    return res
      .status(200)
      .send(JSON.stringify({ success: true, data: { users: Types } }, null, 3));
  } catch (err) {
    log.logger.error(
      `{"verb":"${req.method}", "path":"${
        req.baseUrl + req.path
...
router.post("/", async (req, res) => {
  try {
    ...
    connection.query(
      "INSERT INTO usuarios(usuario, clave, esAdmin, estado) VALUES(?, ?, 'SI', 'AC')",
      values,
      (err, result) => {
        if (err) {
